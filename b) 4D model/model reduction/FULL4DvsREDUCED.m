...
sys_dyn = @(t,y) [(((p.c+2*y(1))*((p.u10+p.u1+p.a*(y(1))+p.abar*(y(1)))+(p.u20+p.u2+p.aprime*(y(1)))))*y(2)...
                 -(p.mu*(p.b*p.c+y(2))*p.eprime*p.mup*(p.beta*p.c+y(2))*((1/((p.u10+p.aprime*(y(1)))))+(1/((p.u20+p.a*(y(1))+p.abar*(y(1))))))*(p.ua0+p.ua+y(2))*y(1)))...
                 /(p.ua0+p.ua+y(2)+p.u10+p.u1+p.a*(y(1))+p.abar*(y(1))+p.u20+p.u2+p.aprime*(y(1)));...
...
sys_dyn = @(t,y) [(p.u1+p.u10+p.aprime*(y(2)+y(3)))*y(4)*(1/p.eprime)+p.mu*(p.b*p.c+y(5))*y(3)-((p.u20+p.a*(y(2)+y(3))+p.abar*(y(1)+y(3)))*(1/p.eprime)+p.mup*(p.beta*p.c+y(5)))*y(1);...
                  ...
                  (p.mup*(p.beta*p.c+y(5))*y(1)+p.mu*(p.b*p.c+y(5))*y(2)+(p.c+y(1)+y(2)+y(3)+y(3))*y(5))-(1/p.eprime)*((p.u2+p.u20+p.a*(y(2)+y(3))+p.abar*(y(1)+y(3)))+(p.u1+p.u10+p.aprime*(y(2)+y(3)))+p.ua+p.ua0+y(5))*y(4);...
                  (p.ua+p.ua0+y(5))*y(4)*(1/p.eprime)-(p.c+y(1)+y(2)+y(3)+y(3))*y(5)];
